...
Lastly, to try and identify any very specific outliers, the data were divided into bins separated by each 1/3 of each month of the season, pre-NBA experience group, and draft position (first round, second round, or undrafted) for a total of 551 bins.  With so many distinct categories, any given outlier can heavily skew the data, so only the middle 50% were analyzed (25th pctl to 75th pctl).  Some categories have no data because no data fit such conditions, mainly due to few games played in such conditions.  Below is a heat map of the middle 50% of Game Scores for all 551 bins.
```{r,echo=FALSE,warning=FALSE,message=FALSE}
library(plotly)
...
ggplot(
  ...
  legend.justification = c(0.5, 1), legend.position = c(1, 1)
)
...
The first interesting observation is that February seems to possess some of the reddest bins on the entire map, especially in the middle third of the month for international players.  This is relatively contradictory to the line graphs, which could indicate a right-skewed trend, with few very high Game Score outliers that were removed above the 75th percentile mark, but a lot of very low scores below the 25th.  Each bin possesses only about 5-25 games, so the data are still of a fairly small sample to tell a relatively informative story.

## Conclusion
First and foremost, the concept of a "Rookie Wall" really does seem to exist in the NBA to a significant degree, however surprisingly, college players seem to be less volatile than international players, possibly a result of the lack of standardization amongst the level of international leagues.  Unsurprisingly, college seniors, who steadily improve throughout the year, are much more consistent than freshmen, whose wall exist, but not until about a month later than most others.  Whether the wall "hits" in February or March though, most everybody rebounds around April.

Regardless, nothing is permanent enough to substantially affect draft decisions and truly alter the way organizations are conducted, because drafting any type of rookie will likely result in at least some sort of natural regression, and that's just the name of the game.  With that in mind, the number one thought to take away from this paper is that all rookies and all athletes are humans just like everyone else, and for as talented as they are, deserve to be treated with fair and reasonable levels of scrutiny and criticism.
